...
function displayWeather() {
	//console.log(weather_dataset);
	if(weather_dataset == undefined) return;
	if(weather_dataset.location == undefined) return;

	$('#weather > table:nth-child(1) > tbody > tr:nth-child(1) > td:nth-child(2)').html(weather_dataset.location + '<img src="/image/place.png" alt="">');
	$('#weather > table:nth-child(1) > tbody > tr:nth-child(2) > td:nth-child(1)').html('<img src="/image/weather/' + weather_dataset.icon + '.png" alt="' + weather_dataset.sky + '">');
	$('#weather > table:nth-child(1) > tbody > tr:nth-child(2) > td:nth-child(2)').html(weather_dataset.currentTemperature + '˚');
	$('#weather > table:nth-child(1) > tbody > tr:nth-child(3) > td:nth-child(2)').html('최저 ' + weather_dataset.minTemperature + '˚');
	$('#weather > table:nth-child(1) > tbody > tr:nth-child(4) > td:nth-child(1)').html('최고 ' + weather_dataset.maxTemperature + '˚');
	$('#weather > table:nth-child(1) > tbody > tr:nth-child(4) > td:nth-child(1)').html(weather_dataset.sky);
	$('#weather > ul > li:nth-child(1) > span').html(weather_dataset.finedust);
	if(weather_dataset.finedust.indexOf("좋음") >= 0) {
		$('#weather > ul > li:nth-child(1) > span').addClass("green");
		$('#weather > ul > li:nth-child(1) > span').removeClass("red");
	} else if(weather_dataset.finedust.indexOf("나쁨") >= 0) {
		$('#weather > ul > li:nth-child(1) > span').removeClass("green");
		$('#weather > ul > li:nth-child(1) > span').addClass("red");
	} else {
		$('#weather > ul > li:nth-child(1) > span').removeClass("green");
		$('#weather > ul > li:nth-child(1) > span').removeClass("red");
	}
	...
	if(weather_dataset.ultrafinedust.indexOf("좋음") >= 0) {
		$('#weather > ul > li:nth-child(2) > span').addClass("green");
		$('#weather > ul > li:nth-child(2) > span').removeClass("red");
	} else if(weather_dataset.ultrafinedust.indexOf("나쁨") >= 0) {
		$('#weather > ul > li:nth-child(2) > span').removeClass("green");
		$('#weather > ul > li:nth-child(2) > span').addClass("red");
	} else {
		$('#weather > ul > li:nth-child(2) > span').removeClass("green");
		$('#weather > ul > li:nth-child(2) > span').removeClass("red");
	}
	...
	for(var i=0; i<weather_dataset.lists.length; i++) {
		...
		$('#weather > table:nth-child(3) > tbody > tr:nth-child(' + (i+1) + ') > td:nth-child(2)').html('<img src="/image/weather/' + weather_dataset.lists[i].icon + '.png" alt="' + weather_dataset.lists[i].sky + '"/>');
		$('#weather > table:nth-child(3) > tbody > tr:nth-child(' + (i+1) + ') > td:nth-child(3)').html(weather_dataset.lists[i].temperature + '˚');
	}
}
...
function toggleCurtain() {
	$('#curtain-modal').fadeIn();
  
	// 상태 조회 후 슬라이더 초기화
	getCurtainStatus().then(status => {
	  if (status && status.attributes && status.attributes.current_position != null) {
		const position = status.attributes.current_position;
		$('#curtain-slider').val(position);
		$('#curtain-value').text(`${position}%`);
	  } else {
		$('#curtain-slider').val(0);
		$('#curtain-value').text('0%');
	  }
	});
  }
  
  // 전체 열기 버튼
  $('#open-curtain').on('click', async function () {
	await openCurtain();
	$('#curtain-slider').val(100);
	$('#curtain-value').text('100%');
  });
  
  // 전체 닫기 버튼
  $('#close-curtain').on('click', async function () {
	await closeCurtain();
	$('#curtain-slider').val(0);
	$('#curtain-value').text('0%');
  });
  ...
  // 모달 닫기 (배경 클릭 시 닫기)
  $('#curtain-modal').on('click', function (e) {
	if ($(e.target).is('#curtain-modal')) {
	  $(this).fadeOut();
	}
  });
